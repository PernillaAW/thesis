...
class oracleModel{
    ...
    */
    ...
    async insert(path, table) {
        const sql = `LOAD DATA INFILE ${path} INTO TABLE ${table} FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"' (Severity, State, Precipitation, Windy, Start_Lng, Start_Lat, Start_time, End_time)`;
        const result = await dbConnectionOracle.execute(sql);
        return;
    }
