...
routes.post("/insert", async (req, res)=>{
    try {
        const result = await oracle.insert(req.body.collection, req.body.path);
...
routes.post("/read_all", async (req, res) => {
    const result = await oracle.readAll(req.body.collection);
...
routes.post("/read_one", async (req, res) => {
    const result = await oracle.readOne(req.body.collection);
...
routes.post("/delete", async (req, res) => {
    ...
    const result = await oracle.delete(req.body.collection);
    res.status(200).json(result);
})
