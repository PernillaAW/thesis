...
routes.post("/read_all", async (req, res) => {
    const result = await mongoDB.readAll(req.body.collection);
    res.status(200).json(result);
})

routes.post("/read_partial", async (req, res) => {
    const result = await mongoDB.readPartial (req.body.collection, req.body.columnOne, req.body.columnTwo, req.body.valueOne, req.body.valueTwo);
