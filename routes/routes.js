...
routes.post("/insert", async (req, res)=>{
    const result = await pg.insert(req.body.file, req.body.collection);
...
routes.post("/read_all", async (req, res) => {
    const result = await pg.readAll(req.body.collection);
    res.status(200).json(result);
})

routes.post("/read_partial", async (req, res) => {
    const result = await pg.readPartial (req.body.collection, req.body.columnOne, req.body.columnTwo, req.body.valueOne, req.body.valueTwo);
    //const result = await postgis.readPartial(req.body.collection, req.body.valueOne)
    res.status(200).json(result);
})

routes.post("/read_one", async (req, res) => {
    const result = await pg.readOne(req.body.collection);
    res.status(200).json(result);
})

routes.post("/delete", async (req, res) => {
    const result = await pg.delete(req.body.collection);
    res.status(200).json(result);
})
